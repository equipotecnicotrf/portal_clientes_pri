...
public interface ItemsRepository extends JpaRepository <ItemsEntity,Long>{
    @Query(value = "SELECT i FROM ItemsEntity i WHERE  i.Atribute9 <> 0 AND i.inventory_item_id in (SELECT a.inventory_item_id FROM AvailabilityEntity a where a.Available_to_transact <> 0)")
