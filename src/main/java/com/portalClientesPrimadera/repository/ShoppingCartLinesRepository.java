...
public interface ShoppingCartLinesRepository extends JpaRepository <ShoppingCartLinesEntity, Long> {
    ...
    @Query(value = "SELECT s, i, p  FROM ShoppingCartLinesEntity s, ItemsEntity i , PricesEntity p  WHERE s.inventory_item_id = i.inventory_item_id AND i.inventory_item_id = p.Inventory_item_id AND s.CP_cart_id = :cp_cart_id")
    public List<ArrayList> findByitemscpcartid(@Param("cp_cart_id") Long cp_cart_id);
}
